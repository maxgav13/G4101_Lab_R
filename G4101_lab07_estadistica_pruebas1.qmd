...
# usando dataframe
var.test(cty ~ manufacturer, data = mpg,
...
```{r}
# MOTE
d1 = d_single_t(m = mean(verm), u = mu, sd = sd(verm), 
...
```{r}
# MOTE
d2.ind = d_ind_t(m1 = mean(A), m2 = mean(B), 
        sd1 = sd(A), sd2 = sd(B), 
...
```{r}
# MOTE
g2.ind = g_ind_t(m1 = mean(A), m2 = mean(B), 
        sd1 = sd(A), sd2 = sd(B), 
        n1 = length(A), n2 = length(B), a = alfa)
...
```{r}
# MOTE
d2.av = d_dep_t_avg(m1 = mean(Antes),m2 = mean(Despues),
...
```{r}
# MOTE
d2.rm = d_dep_t_rm(m1 = mean(Antes),m2 = mean(Despues),
           sd1 = sd(Antes),sd2 = sd(Despues),
