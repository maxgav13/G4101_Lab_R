...
Este es un documento Quarto (`.qmd`). Permite trabajar con texto y ejecutar codigo de manera conjunta para generar un documento completo de su analisis. Para compilar el documento haga click en *Render* (![](imgs/rstudio-render-button.png){width="20" height="17"}). 
...
Si el analisis en **R** involucra componentes interactivos, estos tambien son compatibles con los resultados en el archivo *html*. Estos **NO** pueden desplegarse en *pdf* o *word*, por lo que se incluyen solo cuando el documento se renderiza a *html*.
...
Siempre que un valor exista dentro de un objeto guardado, este se puede acceder para ser desplegado en el cuerpo del documento. Por ejemplo: La porosidad media es de `r mean(dat$Porosity)`%, que se escribe de la siguiente manera: ``{r} mean(dat$Porosity)``.
...
Otra forma de importar figuras externas (guardadas previamente) es usando la sintaxis de markdown ([Figuras Quarto](https://quarto.org/docs/authoring/figures.html)): ``![descripcion](ubicacion-de-la-imagen){#fig-label fig-align="center" width="60%"}``.

![Paletas de viridis](imgs/viridis.png){#fig-viridis fig-align="center" width="60%"}
