...
marginaleffects::avg_comparisons(t2.welch, variables = list(group_id = "pairwise"), df = insight::get_df(t2.welch))
```
...
$$H_0: y = \beta_0 \\text{ para todos los grupos}$$
...
Con varianza diferente:

```{r}
aov1.welch = nlme::gls(values ~ ind, Stacked,
                     weights = nlme::varIdent(form=~1|ind),
                     method="ML")
...
aov1.welch %>% 
  marginaleffects::avg_comparisons(variables = list(ind = "pairwise"), df = insight::get_df(aov1.welch))
```

```{r}
aov1 = lm(values ~ ind, Stacked)

lmtest::coeftest(aov1, vcov = sandwich::vcovHC(aov1, type = "HC3"))
lmtest::waldtest(aov1, vcov = sandwich::vcovHC(aov1, type = "HC3"))
```
