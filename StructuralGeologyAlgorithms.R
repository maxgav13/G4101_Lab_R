...
SmallCircleD = function (trda, plga, coneAngle, wulff = F) {
  trda = (trda)*pi/180
  plga = (plga)*pi/180
...
StPointD = function (az, iang) {
  ...
  return(tibble(x = pltx, y = plty))
}

dir_3D_eigen = function(trd, plg, a = .05,
                        type = c('line','dir','strike'), 
                        results = c('basic','full'),
                        plot = c('all','eig','mean')) {
  ...
  sig_c = sqrt(d.sig/(N*Rbar^2))
  q = suppressWarnings(asin(sqrt(-log(a))*sig_c)*180/pi)
...
dir_sph_ax_aov = function(trd, plg, grp, a = .05,
                          type = c('line','dir','strike'), 
                          results = c('basic','full')) {
...
dir_sph_vc_aov = function(trd, plg, grp, a = .05,
                          type = c('line','dir','strike'), 
                          results = c('basic','full')) {
