...
```{r}
(R = sqrt(x^2+y^2)) # resultante
(Rbar = sqrt(x^2+y^2)/N) # resultante media
...
hcfcirc.boot(thetarad,theta2rad) # 2
hcf.circaov(thetaall,grps) # 2+

lrcirc.boot(thetarad,theta2rad) # 2
lr.circaov(thetaall,grps) # 2+

# Parametros de concentracion diferentes
hetcirc.boot(thetarad,theta2rad) # 2
het.circaov(thetaall,grps) # 2+
```
