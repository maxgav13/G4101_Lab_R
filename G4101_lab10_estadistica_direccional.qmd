...
meandeg = deg(meanrad) # direccion media en grados
```
...
Diagrama de rosas mostrando la direccion media y cono de confianza
...
theta2rad = rad(theta2)
```

```{r}
thetaall = c(theta,theta2)
thetaallc = circular(thetaall,units = 'degrees')
thetaallrad = rad(thetaall)
...
```{r}
equal.kappa.test(thetaallc, grps)
```
...
aov.circular(thetaallc, grps, method = "LRT") # 2+
