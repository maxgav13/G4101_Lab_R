...
Se usa la transformacion de Box-Cox donde se debe hallar el parametro $\lambda$ para transformar la variable lo mas cercano a una variable con distribucion normal. La transformacion logaritmica (natrual, base $e$) es un caso especial donde $\lambda=0$.
...
if (isTRUE(transformar)) {
  bc = MASS::boxcox(datos[[myvar]] ~ 1)
  bc.lambda = bc$x[which.max(bc$y)]
  # bc.lambda = bc %>% as_tibble() %>% slice_max(y,n = 1) %>% pull(x)
  var.trans = BoxCox(datos[[myvar]], bc.lambda)
  datos[[str_glue('{myvar}.trans')]] = var.trans 
}
```

```{r}
myvar = str_glue('{myvar}.trans') # variable a modelar
...
# error del ajuste
(fit.rmse = sqrt(attributes(dat.fit)$SSErr/(nrow(datos)))) 
...
```{r krigingTG, dependson='box-cox', cache=TRUE}
if (isTRUE(transformar)) {
  okTG = krigeTg(as.formula(paste(str_remove(myvar,'.trans'),'~1')), 
                 locations = datos_sf,
                 newdata = datosint2, 
                 model = varmod,
                 lambda = lambda) 
...
rgdal::writeGDAL(ok_sp['var1.pred'], 'figs/krige_pred.tif')
rgdal::writeGDAL(ok_sp['var1.var'], 'figs/krige_var.tif')
```
...
p1 = ggplot() + 
  ...
  geom_contour(aes(x,y,z=var1.pred), data.frame(ok),
...
ok %>% 
  ...
  mapview(layer.name=myvar,col.regions=viridis(10,option = 'D'))
```
