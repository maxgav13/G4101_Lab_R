...
```{r distancias}
dists = st_distance(datos_sf) |> (\(m) m[lower.tri(m)])() |> unclass()
...
```{r}
tm_shape(outline) + 
  tm_borders(fill_alpha = 0.1, col = 'cyan') +
  tm_shape(datos_sf) + 
  tm_symbols(fill = myvar, size = 1, fill_alpha = 0.8, 
...
datos_sf = datos_sf |> 
  left_join(fold_tbl, by = ".point_id") |>
  mutate(fold = factor(fold))
...
tipo.cv = 'spatial' # 'kfold', 'loo' o 'spatial'
...
ggplot() + 
  ...
  geom_sf(data = datos_sf, aes(col = fold), 
          size = 3, alpha = 0.6) + 
  scale_color_brewer("Fold", palette = "Spectral") + 
  labs(x = x_map, y = y_map) + 
  if (!is.na(st_crs(datos_sf))) {
    coord_sf(datum = st_crs(datos_sf))
  }
