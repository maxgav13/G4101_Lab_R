...
* `q`: Para encontrar el cuantil (valor de la variable) a una dada probabilidad
* `r`: Para generar un vector de datos de la distribucion

En el caso de generar datos aleatorios (`r`) es recomendable definir un punto de partida para asegurar siempre el mismo resultado, esto se logra definiendo lo que se llama la semilla (seed) por medio de `set.seed(numero)` y escribiendo un numero dentro de la funcion (ejemplo: `set.seed(4101)`).
