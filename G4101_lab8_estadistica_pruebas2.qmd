...
```{r}
effectsize::eta_squared(aov.fit, ci = 1-alfa)
effectsize::omega_squared(aov.fit, ci = 1-alfa)
```
...
ES = map_dfr(contrastes, 
             ~ effectsize::hedges_g(values ~ ind, data = Stacked %>% 
                                      filter(ind %in% .x) %>% 
...
```{r}
cramers_v(round.dist, ci = 1-alfa)
```
